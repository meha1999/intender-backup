"use client";
import SelectIcon from "public/icons/select.svg";
import Arrow from "public/icons/arrow.svg";
import { useRef, useState } from "react";
import { useOutSideClick } from "@/utils/useOutSideClick";
import { motion } from "framer-motion";
import { selectVariants } from "@/utils/variants";

interface MultipleSelectInputProps {
  placeholder?: string;
  value: any;
  setValue: (value: any) => void;
  data: Array<{ text: string | number; value: any }>;
  order?: "desc" | "asc";
  setOrder?: (value: "desc" | "asc") => void;
  className?: string;
}
const MultipleSelectInput: React.FC<MultipleSelectInputProps> = ({
  placeholder,
  value,
  setValue,
  data,
  order,
  setOrder,
  className,
}) => {
  const ref = useRef(null);
  const [open, setOpen] = useState(false);
  useOutSideClick({ ref: ref, isOpened: open, setIsOpened: setOpen });

  return (
    <div
      className={
        `relative flex w-full cursor-pointer items-center justify-between ${className ? className : "bg-milky"} px-4 py-3.5 text-primary ${open ? "rounded-t-3xl" : "rounded-3xl"} ` +
        className
      }
      onClick={() => setOpen((prev) => !prev)}
      ref={ref}
    >
      <div className="flex gap-2.5">
        {setOrder && order && (
          <SelectIcon
            onClick={(e: Event) => {
              e.stopPropagation();
              setOrder(order === "desc" ? "asc" : "desc");
            }}
            className={`${order === "asc" ? "rotate-180 transform" : ""}`}
          />
        )}
        <div className="flex gap-1 text-sm font-bold">
          {value?.map((item: any, index: number) => (
            <p key={item?.value}>
              {0 !== index && "-"} {item.text}
            </p>
          )) ??
            placeholder ??
            "مرتب‌سازی"}
        </div>
      </div>
      {open ? <Arrow className="rotate-180 transform" /> : <Arrow />}
      <motion.nav
        animate={open ? "open" : "closed"}
        variants={selectVariants}
        initial={false}
        className={
          "!absolute right-0 top-9 z-40 flex !max-h-40 w-full flex-col gap-5 overflow-y-auto rounded-br-3xl bg-milky px-4 py-5 text-sm font-bold text-primary" +
          className
        }
      >
        {data.map((item, index) => (
          <div
            key={index}
            onClick={() => {
              value?.find((i: any) => i.value === item.value)
                ? setValue(value.filter((i: any) => i.value !== item.value))
                : setValue([...(value || []), item]);
            }}
            className={`${value?.find((i: any) => i.value === item.value) ? "text-weeny" : ""}`}
          >
            {item?.text}
          </div>
        ))}
      </motion.nav>
    </div>
  );
};

export default MultipleSelectInput;
